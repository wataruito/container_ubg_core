...
if [ -n "$USER" ]; then
   ...
   if [ -n "$USER_ENCRYPTED_PASSWORD" ]; then
      useradd -m -d $USER_HOME -p $USER_ENCRYPTED_PASSWORD -u $USER_ID $USER
      # > /dev/null
   else
      useradd -m -d $USER_HOME -u $USER_ID $USER
      # > /dev/null
   fi
   ...
   if [ -n $USER_HOME ]; then
      ...
      cd $USER_HOME
   #   mkdir mount_point1
      chown -R ${USER}:${USER_GROUPS} ${USER_HOME}
   fi
